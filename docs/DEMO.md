...
1. **The app (30s).** Open http://boutique-dev.192.168.1.8.nip.io/ — browse, add to cart, checkout.
   Show the confirmation page; the confirmation email is delivered via real SMTP (Gmail) from the
   `smtp-eso` secret.
...
11. **Honesty (30s).** State the limitations: verifyImages in Audit (private-IP Harbor realm, CR-KYVERNO-1),
    Falco removed after a crash-loop, Trivy Operator installed with vuln scanning disabled (capacity),
    Loki absent, one dashboard. This is what a real engineer reports.
